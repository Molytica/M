...
np.random.seed(seed)
random.seed(seed)
dataset = random.sample(dataset, int(float(len(dataset)) * 0.1))  # Shuffling the dataset
...
def prepare_data_parallel(dataset, num_workers=100):
    X, Y = [], []
...
X, Y = prepare_data_parallel(dataset)
...
class FeedForwardModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1625, 512) # 1624 dims input + 1 binary marker
