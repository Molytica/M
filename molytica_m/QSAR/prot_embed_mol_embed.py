...
class CustomTransformerLayer(nn.Module):
    def __init__(self, dim_model, dim_feedforward):
        ...
        self.attention1 = nn.MultiheadAttention(dim_model, 16)
        self.attention2 = nn.MultiheadAttention(dim_model, 8)
        self.attention3 = nn.MultiheadAttention(dim_model, 4)
    ...
    def forward(self, src):
        ...
        src2 = self.norm3(src)
        attn_output, _ = self.attention2(src2, src2, src2)
        src = src + attn_output
        src = src + self.feed_forward2(src)
        
        src2 = self.norm4(src)
        attn_output, _ = self.attention3(src2, src2, src2)
        src = src + attn_output
...
class TransformerModel(nn.Module):
    def __init__(self, input_dim, dim_feedforward, num_classes, num_layers=5):
        ...
        self.out = nn.Linear(dim_feedforward, num_classes)
...
input_dim = 1624  # Adjust this based on your input feature size
dim_feedforward = 512  # Size of the feedforward network model
