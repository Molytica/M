...
from molytica_m.data_tools import general_data_tools
import random
import json
...
sub_graph_nodes = general_data_tools.get_subgraph_nodes_from_edgelist(edge_list)
...
for key in nodes.keys():
    DLiP_ids[key] = general_data_tools.get_DLiP_ids_from_nodes(nodes[key], DLiP_data)
    uniprots[key] = general_data_tools.get_uniprots(DLiP_ids[key], DLiP_data)
...
for key in nodes.keys():
    PPI_molecules = general_data_tools.get_PPI_molecules(DLiP_ids[key], DLiP_data, af_uniprots)
    general_data_tools.save_graphs(PPI_molecules, save_path, key)
